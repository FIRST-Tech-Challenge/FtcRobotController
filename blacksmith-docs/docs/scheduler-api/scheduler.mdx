...
Utility method for roughly benchmarking performance. Does not average, only prints the raw time
each cycle.

The 'afterEach' block now takes in a double representing the time in ms that you can now do whatever
you want with
...
<TabItem value="kt" label="Kotlin">
  ...
  override fun runOpMode() {
      ...
      Scheduler.time(this) {
          telemetry.addData("LoopTime (ms)", it)
          telemetry.update()
      }
  }
  ```
...
or if `listener.hook` is called on them.
