...
export function SchedulerLaunchParamList() {
  return (
    <ParamsList params={[
      { name: 'opmode',    type: 'LinearOpMode', desc: 'The LinearOpMode to run the Scheduler in',                                         typeLink: 'https://ftc-tricks.com/overview-linear-opmode/' },
      { name: 'afterEach', type: 'Runnable',     desc: 'An optional block of code to run after every cycle, after the listeners have ran', typeLink: 'https://stackoverflow.com/a/68116405', defaultsTo: '() -> {}' }
    ]}/>
  )
}
...
export function SchedulerBeforeEachParamList() {
  return (
    <ParamsList params={[
      { name: 'block', type: 'Runnable', desc: 'The block of code to run at the start of each Scheduler loop', typeLink: 'https://stackoverflow.com/a/68116405' }
    ]}/>
  )
}
...
export function SchedulerTimeParamList() {
  return (
    <ParamsList params={[
      { name: 'opmode',    type: 'LinearOpMode', desc: 'The LinearOpMode to run the Scheduler in',                                        typeLink: 'https://ftc-tricks.com/overview-linear-opmode/' },
      { name: 'telemetry', type: 'Telemetry',    desc: 'The Telemetry object to log the times to - NOTE: will be updated at end of loop', typeLink: 'https://gm0.org/en/latest/docs/software/tutorials/using-telemetry.html' },
      { name: 'afterEach', type: 'Runnable',     desc: 'An optional block of code to run every tick, after the listeners have ran',       typeLink: 'https://stackoverflow.com/a/68116405', defaultsTo: '() -> {}' }
    ]}/>
  )
}
