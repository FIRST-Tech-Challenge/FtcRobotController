...
:::tip
I **highly** recommend using the second method, even though it takes a couple extra steps
:::
...
In the `raw` folder, create a new folder called `blacksmith` and in there, put in a `units.properties` file
...
To get the spellings for each supported unit, find the doc for the designed unit (e.g. [`DistanceUnit`](./units/distance)),
