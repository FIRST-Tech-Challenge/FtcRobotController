...
public class Odometry {
    ...
    public void runOdom(){
        double left_encoder_pos = leftEncoder.getCurrentPosition()/(360*6.0) * wheel_circumference_inches;//inches
        double right_encoder_pos = rightEncoder.getCurrentPosition()/(360*6.0) * wheel_circumference_inches;
        double perpendicular_encoder_pos = perpendicularEncoder.getCurrentPosition()/(360*6.0) * wheel_circumference_inches;
        ...
        double delta_x_integrated = Math.cos(heading) * ((Math.sin(phi)/phi)*delta_middle_pos + (Math.cos(phi)-1)/phi * delta_perp_pos) -
                Math.sin(heading)*((1-Math.cos(phi))/phi * delta_middle_pos + (Math.sin(phi)/phi)*delta_perp_pos);

        double delta_y_integrated = Math.sin(heading)*((Math.sin(phi)/phi)*delta_middle_pos + (Math.cos(phi)-1)/phi * delta_perp_pos) +
