...
//Based on testing, the front and back left motors were reversed.  Reversing them here (FIXED THE ISSUE!!!)
        ...
        intakeMotor.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.FLOAT);   //allow to spin if it rolls onto something
        ...
        robotRaiserMotor.setZeroPowerBehavior(DcMotor.ZeroPowerBehavior.BRAKE);
...
//            float robotRaiseDownMotorPower = gamepad1.left_trigger;
            ...
            if (gamepad2.left_trigger > 0) {
               // if(Math.abs(lifterMotor.getCurrentPosition()) < 1025) {
                if(lifterMotor.getCurrentPosition() > -1025) {
                    //pixellifterRaise
                    lifterMotor.setDirection(DcMotorSimple.Direction.REVERSE);
                    lifterMotor.setPower(0.3 * (lifterLowerMotorPower));
                } else {
                    lifterMotor.setPower(0); //at or beyond limit so don't move it
                }
            } else if (gamepad2.right_trigger > 0) { //don't drive motor down if already on the ground!
                //pixellifter - Lower
                if (lifterMotor.getCurrentPosition() < 0) { //positive value means pushing into the floor
