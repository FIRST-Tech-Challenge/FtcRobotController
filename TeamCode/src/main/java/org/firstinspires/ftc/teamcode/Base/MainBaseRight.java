...
public class MainBaseRight {
    ...
    public VisionRight detectorRight = null;
    ...
    public void init(HardwareMap ahwMap, OpMode gievnOpMode) {
        ...
        detectorRight = new VisionRight(opMode);
        opMode.telemetry.addLine("Camera Right");
        opMode.telemetry.update();
...
//
    ...
    public void findLine(double holdTime, LinearOpMode opMode){
        ...
        holdTimer.reset();
        while (color.blue() <= 270  && color.red() <= 200 && holdTimer.time() < holdTime) {
...
//           }
    ...
    public void navxgyroTurn(double speed, double angle, LinearOpMode opMode) {
        ...
        // Update telemetry & Allow time for other processes to run.
        navxonHeadingTurn(speed, angle, P_TURN_COEFF);
        opMode.telemetry.update();
...
//        telemetry.addData("Speed.", "%5.2f:%5.2f", leftSpeed, rightSpeed);
    ...
    boolean navxonHeadingTurn(double speed, double targetAngle, double PCoeff) {
        double error;
        double steer;
        boolean onTarget = false;
        double leftSpeed;
...
//        yawPIDController.setContinuous(true);
        ...
        while(!((navx_device.getYaw() <= targetAngle + 1) && (navx_device.getYaw() >= targetAngle - 1))){
            opMode.telemetry.addData("Current Yaw", navx_device.getYaw());
            opMode.telemetry.addData("Error", yawPIDController.getError());
            opMode.telemetry.addLine("IN LOOP");

            if (navx_device.getYaw() <= targetAngle + 1){
