...
public class RFMotionProfile {
    double curviness, c, velo1, velo2, endVel, startVel, startT, MAX_ACCEL, MAX_VEL;
    ...
    boolean currentlyDeceling = false;
    double FFTime = 0.1;

    public RFMotionProfile(double p_startVel, double p_endVel, double p_curviness, double p_MAX_ACCEL, double p_MAX_VEL) {
        MAX_VEL = p_MAX_VEL;
        MAX_ACCEL = p_MAX_ACCEL;
        ...
        LOGGER.log("c: "+c);
        endVel = min(p_endVel, MAX_VEL);
        startVel = min(p_startVel, MAX_VEL);
    ...
    public double getInstantaneousTargetAcceleration(double dist,double projectedVelo, double endVelo) {
//        velo = currentVelocity.vec().norm();
//        double p_vel = velo;
        double targetAccel = 0;
        //if u should decel
        boolean shouldDecel = false;
        double decelDist = getAccelDist(projectedVelo, endVelo);
        ...
        if (velo < velo1) {
            ...
            packet.put("accelMagCase2: "+ velo2, velo);
...
//                    packet.put("remDist3", totalDist - startDist - c * t * t * t / 6);
            ...
            double peakVel = MAX_ACCEL*(tList.get(6)-tList.get(5));
            if (time > tList.get(5)) {
                double t2 = tList.get(6) - time;
                packet.put("remDist3", totalDist - MAX_ACCEL * t2 * t2 * 0.5);

                return totalDist - startDist - MAX_ACCEL * t2 * t2 * 0.5;
            } else if (time > tList.get(3)) {
                double t2 = tList.get(6) - tList.get(5);
                double t4 = tList.get(4) - time;
                packet.put("remDist3", totalDist - MAX_ACCEL * t2 * t2 * 0.5 - peakVel * t4);

                return totalDist - startDist - MAX_ACCEL * t2 * t2 * 0.5 - peakVel * t4;
            } else {
                double t2 = tList.get(6) - tList.get(5);
                double t4 = tList.get(4) - tList.get(3);
                double t6 = tList.get(3) - time;
                packet.put("totalDist", totalDist);
                packet.put("startDist", startDist);
                packet.put("peakVel", peakVel);
                packet.put("remDist1", - peakVel * t6 + MAX_ACCEL * t6 * t6 * 0.5);
                packet.put("remDist2", totalDist - startDist - MAX_ACCEL * t2 * t2 - peakVel * t4);
                packet.put("remDist3", totalDist- startDist - MAX_ACCEL * t2 * t2 * 0.5 - peakVel * t4 - peakVel * t6 + MAX_ACCEL * t6 * t6 * 0.5);
                return totalDist - startDist - MAX_ACCEL * t2 * t2 * 0.5 - peakVel * t4 - peakVel * t6 + MAX_ACCEL * t6 * t6 * 0.5;
            }
        }
    }
    ...
    public double veloDecelDist(double vel) {
        ...
        if(Double.isNaN(dist)){
           LOGGER.log("veloDecelDist NaN, c :" + c);
        }
        return dist;
    }
    ...
    public double getAccelDist(double vel1, double vel2) {
        if (curviness != 0) {
            ...
            double t = abs((vel2 - vel1) / MAX_ACCEL);
            int direction = 1;
            if(vel2<vel1)
                direction=-1;
            return vel1 * t + direction * 0.5 * MAX_ACCEL * t * t;
        }
    }
}
