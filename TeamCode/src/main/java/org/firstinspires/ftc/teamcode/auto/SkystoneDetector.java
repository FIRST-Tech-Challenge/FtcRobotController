...
public class SkystoneDetector extends OpenCvPipeline {
    ...
    public Mat processFrame(Mat input) {
        ...
        Scalar YellowHighHSV = new Scalar(27, 255, 255); // higher bound HSV for yellow

        Scalar PinkLowHSV = new Scalar(295, 43, 100); // lower bound HSV for yellow
        Scalar PinkHighHSV = new Scalar(300, 255, 255); // higher bound HSV for yellow

        Scalar GreenLowHSV = new Scalar(112, 100, 90); // lower bound HSV for yellow
        Scalar GreenHighHSV = new Scalar(117, 255, 255); // higher bound HSV for yellow

        Mat yellowThresh = new Mat();
        Mat pinkThresh = new Mat();
        Mat greenThresh = new Mat();

        // We'll get a black and white image. The white regions represent the regular stones.
        // inRange(): yellowThresh[i][j] = {255,255,255} if mat[i][i] is within the range
        Core.inRange(mat, YellowLowHSV, YellowHighHSV, yellowThresh);
        Core.inRange(mat, PinkLowHSV, PinkHighHSV, pinkThresh);
        Core.inRange(mat, GreenLowHSV, GreenHighHSV, greenThresh);
        // Use Canny Edge Detection to find edges
        // you might have to tune the thresholds for hysteresis
        Mat edges = new Mat();
        Imgproc.Canny(yellowThresh, edges, 100, 300);
        Imgproc.Canny(greenThresh, edges, 100, 300);
        Imgproc.Canny(pinkThresh, edges, 100, 300);
