...
public class ColorAndOrientationDetect implements VisionProcessor {
    ...
    private final List<DetectedAngle> lastValidDetections = new ArrayList<>();
    ...
    public Mat processFrame(Mat input, long captureTimeNanos) {
        List<DetectedAngle> newDetections = new ArrayList<>(); // Temporary list for current frame
        ...
        try {
            ...
            processMask(ycrcb, hsvMat, input, lowerBlue, upperBlue, "Blue", new Scalar(255, 0, 0), newDetections); // Blue
            processMask(ycrcb, hsvMat, input, lowerRed, upperRed, "Red", new Scalar(0, 0, 255), newDetections);   // Red
            processMask(ycrcb, hsvMat, input, lowerYellow, upperYellow, "Yellow", new Scalar(0, 255, 255), newDetections); // Yellow
        ...
        // Safely update detectedColors and lastValidDetections
        synchronized (detectedColors) {
            detectedColors.clear();
            detectedColors.addAll(newDetections);

            if (!newDetections.isEmpty()) {
                synchronized (lastValidDetections) {
                    lastValidDetections.clear();
                    lastValidDetections.addAll(newDetections);
                }
            }
        }

        return input; // Return the modified frame with annotations
    }

    private void processMask(Mat ycrcb, Mat hsvMat, Mat frame, Scalar lower, Scalar upper, String colorName, Scalar boxColor, List<DetectedAngle> newDetections) {
        Mat mask = new Mat();
        Core.inRange(ycrcb, lower, upper, mask);

        try {
            processOrientation(mask, hsvMat, frame, colorName, boxColor, newDetections);
        } finally {
            mask.release(); // Release the mask to free memory
        }
    }

    private void processOrientation(Mat mask, Mat hsvMat, Mat frame, String colorName, Scalar boxColor, List<DetectedAngle> newDetections) {
        ...
        try {
            ...
            for (MatOfPoint contour : contours) {
                ...
                if (boundingBoxArea > MIN_BOUNDING_BOX_AREA) {
                    ...
                    newDetections.add(detectedAngle);
                }
            }
        } finally {
            hierarchy.release();
    ...
    public double calAngle(String colorName) {
        ...
        synchronized (lastValidDetections) {
            for (DetectedAngle detected : lastValidDetections) {
                if (detected.getColorName().equalsIgnoreCase(colorName)) {
                    return detected.getAngle();
                }
            }
        ...
        return -999.99; // Return default if no detections exist
    }
    ...
    public double[] calCenter(String colorName) {
        ...
        return new double[0]; // Return empty if no detections exist
    }
...
//            return -999.99; // Return null if no objects are detected
//        }
//        for (DetectedAngle detected : detectedColors) {
//            if (detected.getColorName().equalsIgnoreCase(colorName)) {
...
//            }
//        }
//        return -999.99; // Return null if no object of the specified color is found
//    }
//
//    // Public Method: Get center coordinates of the specified color
//    public double[] calCenter(String colorName) {
//        if (detectedColors.isEmpty()) {
//            return new double[0]; // Return null if nothing is detected
//        }
//        for (DetectedAngle detected : detectedColors) {
//            if (detected.getColorName().equalsIgnoreCase(colorName)) {
//                Point center = detected.getCenter();
...
//            }
//        }
//        return new double[0];  // Return null if the color is not found
//    }
//
