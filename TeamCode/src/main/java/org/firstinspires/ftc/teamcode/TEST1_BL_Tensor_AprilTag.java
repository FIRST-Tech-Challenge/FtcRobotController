...
@Autonomous(name = "Robot: April Tag Auto Blue Left", group = "Robot")
//@Disabled
public class TEST1_BL_Tensor_AprilTag extends LinearOpMode {
...
//        private DcMotor rearRight = null;  //  Used to control the right back drive wheel
    ...
    public void runOpMode() {
        ...
        telemetry.update();
        encoderDrive(0.65, 21.5, 21.5, 21.5, 21.5, 3);//tested with higher speed but then we need to reduce distance
        ...
        while (opModeIsActive()) {
            ...
            if (sensorDistanceLeft.getDistance(DistanceUnit.INCH) < 6){
                DESIRED_TAG_ID = 1;
            ...
            if (sensorDistanceRight.getDistance(DistanceUnit.INCH) < 12) {
                DESIRED_TAG_ID = 3;
        ...
        if (DESIRED_TAG_ID == 2) {
            encoderDrive(0.65, 16, 16.0, 16, 16, 5.0);  // S1: backward 27 Inches with 5 Sec timeout
            ...
            encoderDrive(0.35, 3, 3, 3, 3, 2.0);

            runMotorsWithTurning(0, 0, 90); //rotate clockwise 180 degree

            encoderDrive(0.65, 20, 20, 20, 20, 5);
            runMotorsForTime(0,0.3,0,2);//strafe left
            //maybe add strafing to view apriltag
            encoderDrive(0.6, 17.5, 17.5, 17.5, 17.5, 4);
...
//new place code*********************
            ...
            runMotorsForTime(0,0.3,0,0.5);//park to the left of the field
            //lower the lift
            liftspeed=0.75;
            liftRight.setPower(-liftspeed);
            liftLeft.setPower(-liftspeed);
            ...
            IntakeWheel.setPower(0);
            sleep(50);
            encoderDrive(0.65, 24, 24, 24, 24, 2);
            sleep(50);
            encoderDrive(0.65, 14, 14, 14, 14, 4);
           //new place code*********************
            //drop the pixel on backdrop
            double liftspeed = 0.3;
            //lift goes up with runtime of 2 second
            ...
            //park --move away to allow partner to place then lower the lift
            encoderDrive(0.25, -2, -2, -2, -2, 2);

            runMotorsForTime(0, 0.3, 0, 0.75);//park to the left of the field
            ...
            runMotorsForTime(0, 0.3, 0, 1);//park to the left of the field
            ...
            runMotorsForTime(0,0.3,0,2);

//            DrivetoApriltag(DESIRED_TAG_ID);
//
          encoderDrive(0.65, -24, -24, -24, -24, 3);//                //runMotorsWithTurning(0, 0, -90); //rotate clockwise 90 degree

//          sleep(1000);
          runMotorsForTime(0,-0.3,0,2.3);//
            sleep(500);
            runMotorsWithTurning(0, 0, 90); //rotate Counterclockwise 180 degree
            //
            encoderDrive(0.65, 15, 15, 15, 15, 2);//                //runMotorsWithTurning(0, 0, -90); //rotate clockwise 90 degree
//new place code*********************
            //drop the pixel on backdrop
            double liftspeed = 0.3;
            //lift goes up with runtime of 2 second
            ...
            servoPlacer.setPosition(0.65*MAX_POS);
            sleep(500);
            //park--start move away to allow more time for partners
            encoderDrive(0.25, -2, -2, -2, -2, 2);

            runMotorsForTime(0, 0.45, 0, 0.7);//park to the left of the field
            //lower the lift
            liftspeed = 0.5;
            liftRight.setPower(-liftspeed);
            liftLeft.setPower(-liftspeed);
