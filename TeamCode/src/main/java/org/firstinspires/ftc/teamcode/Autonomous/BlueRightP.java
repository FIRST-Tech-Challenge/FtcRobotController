...
//        double[] turretTarget = {12+10.6,-24+16.2,0}; //{hubx-position*3/2,huby-position*3/2,1+7*position}
        ...
        waitForStart();
        position = 0;
        robot.goToPosition(0, -15, 0, 0, 0.5);
        if(position==1) {
            robot.TurretSlidesToPosition(-30, -30, 0, 0.5);
            sleep(1300);
        }
        if (position==0) {
            robot.TurretSlidesToPosition(-25, 20, 0, 0.5);
            sleep(5000);
        }
        else {
            robot.TurretSlidesToPosition(-30, 18, 15, 0.5);
            sleep(1300);
        }
        robot.FlipBasketToPosition(0.4);
        robot.TurretSlidesToPosition(0, 0, 0, 0.5);
        robot.FlipBasketArmToPosition(0.00);
        robot.goToPosition(1, -5, -25, 40, 0.4);
        robot.spinCarouselAutonomousBlue();
        robot.goToPosition(0, -40, -20, 0, 0.4);
