...
public class ToboBeta extends Logger<ToboBeta> implements Robot2 {
    ...
    public ToboMech.TargetZone tZone = ToboMech.TargetZone.UNKNOWN;
    ...
    public void configure(Configuration configuration, Telemetry telemetry, ProgramType autoside) throws FileNotFoundException {
        ...
        simEventFile = AppUtil.getInstance().getSettingsFile("ToboBeta_events.txt"); // at First/settings directory
...
//        }
    ...
    public void mainTeleOp(EventManager em) {
        ...
        if (grabber != null)
            grabber.setupTelemetry(telemetry);
        if (cameraDetector != null)
...
//
    ...
    public void showStatus(double init_time) throws InterruptedException {
        ...
        telemetry.addData("Config._3", "Grabber=%s|Shooter=%s|Intake=%s|Hopper=%s",
                (useGrabber ? "Yes" : "No"), (useShooter ? "Yes" : "No"), (useIntake ? "Yes" : "No"), (useHopper ? "Yes" : "No"));
        if (chassis != null) {
    ...
    public void initAfterStart() {
        ...
        if (grabber != null && tZone != ToboMech.TargetZone.UNKNOWN) // during autonomous
            grabber.armUp();
        initializeGPSThread();
        if (chassis!=null) {
            batteryVolt = getBatteryVoltage();
...
//        return;
        if (cameraDetector == null) {
            tZone = ToboMech.TargetZone.ZONE_A; // assuming zone_A for simulation purpose
            return;
        }
    ...
    public void deliverFirstWobbleGoalBeta() throws InterruptedException {
        ...
        if (side == ProgramType.AUTO_BLUE) {
            ...
            } else if (tZone == ToboMech.TargetZone.ZONE_C) {//4
    ...
    public void deliverSecondWobbleGoalBeta() throws InterruptedException {
        ...
        if (side == ProgramType.AUTO_BLUE) {
            if (tZone == ToboMech.TargetZone.ZONE_A) {//0
                chassis.driveTo(auto_chassis_power, 75, 200, chassis.odo_heading(), false, 3);
                chassis.rotateTo(1,100,2);
            } else if (tZone == ToboMech.TargetZone.ZONE_B) {//1
    ...
    public void doPowerShots() throws InterruptedException {
        if (tZone == ToboMech.TargetZone.ZONE_A) {
...
//        }
    ...
    public void doHighGoalsAndPowerShots(int numHighGoals, int numPowerShots, boolean keepPos) throws InterruptedException {
        ...
        if (!keepPos) {
            if (tZone == ToboMech.TargetZone.ZONE_B || tZone == ToboMech.TargetZone.ZONE_C && numHighGoals == 3) {
                chassis.driveTo(.55, side(50), 170, 0, true, 4);
            } else if (tZone != ToboMech.TargetZone.UNKNOWN) {
        ...
        if (shooter!=null) {
            if (tZone == ToboMech.TargetZone.UNKNOWN)
    ...
    public void getBonusRingsBeta() throws InterruptedException{
        if (side == ProgramType.AUTO_BLUE) {
            if (tZone == ToboMech.TargetZone.ZONE_A) {//0
                ...
                chassis.driveTo(auto_chassis_power, side(90), 155, 0, false, 2);
    ...
    public void park() throws InterruptedException {
        if (tZone== ToboMech.TargetZone.ZONE_A){
            chassis.driveTo(auto_chassis_power, side(90), chassis.odo_y_pos_cm(), 0, true, 2);
        }
        if (tZone== ToboMech.TargetZone.ZONE_C){
            return;
        }
