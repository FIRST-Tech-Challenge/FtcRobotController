...
public class CASH_Drive_Library {
    ...
    //Calculate_Power
    //Used to determine the motion of the wheels when joystick1 is used
    public double Caclulate_Power(double stick_x, double stick_y, boolean isLFRR)
    {
        double angle;
        double magnitude;
        double result;

        angle = Math.atan2(stick_x, stick_y);
        magnitude = Math.sqrt((stick_x * stick_x) + (stick_y * stick_y));

        if(isLFRR) {
            result = Math.sin(angle - (.25 * 3.14)) * magnitude;
        }
        else {
            result = -Math.sin(angle + (.25 * 3.14)) * magnitude;
        }
    ...
    public void MoveReverse(double power, int distance) {
        //Autonomous
        if (distance > 0) {
            int position = GetTicksCountForMovement(distance);
            InitEncoderForPosition();
    ...
    public int MoveRight(double power, int distance) {
        //Autonomous
        if (distance > 0) {
            int position = GetTicksCountForMovement(distance);
            InitEncoderForPosition();
    ...
    public int MoveLeft(double power, int distance) {
        //Autonomous
        if (distance > 0) {
            int position = GetTicksCountForMovement(distance);
            InitEncoderForPosition();
    ...
    private void InitEncoderForPosition()
    {
