...
public abstract class myOpenCvPipeline {
    ...
    Pair<Mat, Integer> processFrameInternal(Mat input)
    {
        if(isFirstFrame)
        {
            init(input);
        ...
        Pair<Mat, Integer> ret = processFrame(input);
        leakDetection();
        return ret;
    }
    ...
    public abstract Pair<Mat,Integer> processFrame(Mat input);
