...
public class ComboGrabber extends Logger<ComboGrabber> implements Configurable {
    ...
    private double ARM_UP_AUTO = 0.4;
    private double ARM_UP_AUTO_LOW = 0.7;
    ...
    public void armUpLow() {
        if (arm==null) return;
        arm.setPosition(ARM_UP_AUTO_LOW);
        armIsLow = false;
    }
...
//        }
    ...
    public void readyGrabCombo() {
        final String taskName = "ready Grab Combo";
        if (!TaskManager.isComplete(taskName)) return;
        if (slider != null) {
            TaskManager.add(new Task() {
                @Override
