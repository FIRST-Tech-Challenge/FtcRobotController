...
//        towerHeightPipeline = new TowerHeightPipeline(hwMap, vuforia);
    ...
    public boolean calibratePartTwo() {
        switch (calibrateOtherStage) {
            ...
            case 2:
                if(!isBlue) {
                    if (rotateIMU(270, 2.0)) {
                        if(luncher.setElbowTargetPos(460, 1.0)) {
                            calibrateOtherStage++;
                        }
                    }
                }
            ...
            case 3:
                ...
                if(luncher.extendToPositionUnsafe(0, 1)) {
                    luncher.toggleGripper();
    ...
    public boolean recockStoneGrabber() {
        switch (grabState) {
            case 0:
                luncher.servoGripper.setPosition(2200);
                grabTimer = futureTime(3);
                grabState++;
            case 1:
                if (System.nanoTime() >= grabTimer) {
                    luncher.servoGripper.setPosition(1500);
                    grabState = 0;
                    return true;
                }
                break;
    ...
    public boolean retrieveStone(boolean endsAtNorth) {
        switch (craneArticulation) {
            case 0:
                if (luncher.getElbowCurrentPos() < luncher.elbowMid)
                    luncher.setElbowTargetPos(luncher.elbowMid, 1); //make sure were not dragging the stone across the floor
                luncher.setGripperSwivelRotation(luncher.swivel_Front);
                luncher.extendToPosition(luncher.extensionBridgeTransit, 1.0); //gets it in very close so we don't strain the arm
            ...
            case 2:
                if (System.nanoTime() >= retrieveTimer) {
                    luncher.setElbowTargetPos(luncher.elbowBrigeTransit, 1); //sets it down for transition to base
                    craneArticulation = 0;
                    return true;
                }
        }
    ...
    public boolean YoinkStone() { // this goes down and grabs the block note how it assumes it is over the block
        ...
        switch (yoinkStage) {
            case 0:
                if (luncher.setElbowTargetPos(ticksTheElbowShouldGoBasedOnVoltageReading, .5)) {
                    yoinkStage++;
                }

                break;
            case 1:
                if (luncher.setElbowTargetPos(0, 1)) {
                    yoinkStage++;
                }
                break;
            case 2:
    ...
    public boolean autoExtendToTowerHeightArticulation() {
        ...
        luncher.extendToTowerHeight(getDistForwardDist(), towerHeightPipeline.blocks);
        return true;
    }
...
//    }
    ...
    public boolean RotateToFaceStone(){
        switch(RotateToFaceStoneStage) {
            case 0:
                if (isBlue)
                    luncher.setGripperSwivelRotation(luncher.swivel_left_Block);
                else
                    luncher.setGripperSwivelRotation(luncher.swivel_Right_Block);
                luncher.setElbowTargetPos(270,.5);
                luncher.extendToPosition(1100,.5);
                RotateToFaceStoneStage++;
                break;
            case 1:
                if (turret.rotateUntil(!isBlue, luncher.alignGripperForwardFacing())) {
                    miniTimer = futureTime(2);
                    RotateToFaceStoneStage++;
                }
                break;
            case 2:
                luncher.setElbowTargetPos(50, .5);
                if (System.nanoTime() >= miniTimer) {
                    luncher.setElbowTargetPos(300,.5);
    ...
    public boolean shootOut() {
        switch (shootStage) {
            case 0:
                if (luncher.setElbowTargetAngle(30)) {
                    miniTimer = futureTime(1);
                    shootStage++;
                }
                break;
            case 1:
                if (System.nanoTime() >= miniTimer) {
                    luncher.setExtendABobLengthMeters(3.28);
                    shootStage++;
                }
                break;
            case 2:
    ...
    public boolean shootOutPartII() {
        switch (shootStage) {
            ...
            case 1:
                if (System.nanoTime() >= miniTimer) {
                    luncher.setExtendABobLengthMeters(6.56);
                    shootStage++;
                }
                break;
            case 2:
        ...
        switch (miniStateRetTow) {
            ...
            case (1):
                ...
                if (System.nanoTime() >= retractTimer) {
                    ...
                    luncher.setElbowTargetAngle(luncher.getCurrentAngle() + 15);
                    miniStateRetTow++;
                }
    ...
    public boolean restaccDemo(boolean buttonvalue) {
        ...
        if (restacktoggle) {
            if (luncher.getCurrentTowerHeight() > 0) {
                switch (restackStage) {
                    case 0:
                        extendToTowerHeightArticulation();
                        luncher.changeTowerHeight(-1);
                        extendToTowerHeightArticulation();
                    ...
                    case 2:
                        if (System.nanoTime() >= restackTimer) {
                            if (driveForward(atLeft, .1, .3)) {
                                luncher.changeTowerHeight(auxTowerHeight);
                                restackTimer = futureTime(1);
                                restackStage++;
                            }
                        }
