...
@Autonomous(name = "LeftConeCycle", group = "")
public class LeftConeCycle extends LinearOpMode {
    //test1
...
*/
    ...
    public void runOpMode() throws InterruptedException {
        ...
        while (((currTime - startTime) < 30000)&& !done && opModeIsActive()) {
            ...
            switch (resultROI) {
                case 1:
                    // Far left-- parking spot for 1
                    beginAuto();
                    done=true;
                    break;
                case 2:
                    // Middle-- parking spot for 2
                    beginAuto();
                    done=true;
                    break;
                case 3:
                    // Far right-- parking spot for 3
                    beginAuto();
                    done=true;
                    break;
            }
    ...
    private void beginAuto() throws InterruptedException {
        //Strafe 4 inches to the right to become centered in the square
        moveUtils.strafeBuddy(2);
        //Drive forward with pre-loaded cone to knock the signal sleeve cone out of the way-- Move slower to not kick cone out of way
        moveUtils.goStraight(54,.6f,MIN_SPEED,ACCEL);
        //Turn to the right to prepare for cone deposit on high junction
        moveUtils.turnCW(40);
        //Move arm upwards
        actuatorUtils.armPole(3);
        //Drive forward so the arm and cone hover over the junction
        moveUtils.goStraight(4,MAX_SPEED,MIN_SPEED,ACCEL);
        //Drop the cone
        actuatorUtils.gripperOpen(false);
        //Back up so the robot is centered in the square
        moveUtils.goStraight(-6,MAX_SPEED,MIN_SPEED,ACCEL);
        //Move arm to position of top cone in stack
       // actuatorUtils.coneCollect(4);
        //Turn 130` to face the wall and cone stack
        moveUtils.turnCCW(130);
        //Move towards the cone stack
        moveUtils.goStraight(32,MAX_SPEED,MIN_SPEED,ACCEL);
        //Grab the cone
        actuatorUtils.gripperClose(false);
        //Back up
        moveUtils.goStraight(-5,MAX_SPEED,MIN_SPEED,ACCEL);
        //Turn to reach low junction
        moveUtils.turnCW(200);
        //Move forward to be over low junction
        moveUtils.goStraight(4,MAX_SPEED,MIN_SPEED,ACCEL);
        //Back up
        moveUtils.goStraight(-4,MAX_SPEED,MIN_SPEED,ACCEL);
    }
