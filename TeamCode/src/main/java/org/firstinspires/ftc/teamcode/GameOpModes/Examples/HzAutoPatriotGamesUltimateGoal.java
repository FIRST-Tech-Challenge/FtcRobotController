 ...
 */
@Autonomous(name = "Hazmat Patriot Games Ultimate Goal", group = "00-Autonomous" , preselectTeleOp = "Hazmat TeleOp RR Ultimate Goal")
@Disabled
public class HzAutoPatriotGamesUltimateGoal extends LinearOpMode {
        ...
        if (!hzAutonomousController.launchHighGoalOrPowerShot) {
            ...
            if (hzAutonomousController.autoLaunchAim == HzAutonomousControllerUltimateGoal.AutoLaunchAim.HIGHGOAL) {
                ...
                if (HzGameFieldUltimateGoal.playingAlliance == HzGameFieldUltimateGoal.PLAYING_ALLIANCE.BLUE_ALLIANCE) {
                    ...
                    turnAnglePowershot23 = Math.toRadians(af * -6);
                } else {
                    traj = driveTrain.trajectoryBuilder(driveTrain.getPoseEstimate())
            ...
            if (HzGameFieldUltimateGoal.playingAlliance == HzGameFieldUltimateGoal.PLAYING_ALLIANCE.BLUE_ALLIANCE) {
                switch (targetZone) {
                    ...
                    case B:
                        if (!hzAutonomousController.pickRingFromTargetMarker){
                            traj = driveTrain.trajectoryBuilder(driveTrain.getPoseEstimate())
                                    .lineToSplineHeading(new Pose2d(38, af* 20, Math.toRadians(-90)))
                                    .build();
                            driveTrain.followTrajectory(traj);
                        } else {
                            traj = driveTrain.trajectoryBuilder(driveTrain.getPoseEstimate())
                    ...
                    case C:
                        ...
                        traj = driveTrain.trajectoryBuilder(driveTrain.getPoseEstimate())
                                .lineToSplineHeading(new Pose2d(56, af * 43, Math.toRadians(af * -90))) //y:51
                                .build();
                        driveTrain.followTrajectory(traj);
                        break;
                }
    ...
    public void runInnerOnlyLaunchPark(int waitInBetween){
        ...
        traj = driveTrain.trajectoryBuilder(driveTrain.getPoseEstimate())
                .lineToSplineHeading(new Pose2d(50, af * 15, Math.toRadians(af * -45))) //y:51
                .build();
        driveTrain.followTrajectory(traj);
        ...
        if (!hzAutonomousController.launchHighGoalOrPowerShot) {
            ...
            if (hzAutonomousController.autoLaunchAim == HzAutonomousControllerUltimateGoal.AutoLaunchAim.HIGHGOAL) {
                ...
                if (HzGameFieldUltimateGoal.playingAlliance == HzGameFieldUltimateGoal.PLAYING_ALLIANCE.BLUE_ALLIANCE) {
                    ...
                    //PATGAMES : MOVE ROBOT TO LAST LANE and not obstruct to middle lane
                    traj = driveTrain.trajectoryBuilder(driveTrain.getPoseEstimate())
                            .lineToLinearHeading(new Pose2d(-10, -52, Math.toRadians(15)))//-10
                            .build();
                }
                driveTrain.followTrajectory(traj);
    ...
    public void runPickRingsFromTargetMarker(){
        ...
        if (targetZone == HzGameFieldUltimateGoal.TARGET_ZONE.B) {
            traj = driveTrain.trajectoryBuilder(driveTrain.getPoseEstimate())
                    .lineToLinearHeading(new Pose2d(0, af * 34.5, Math.toRadians(af * 180)))
                    .build();
            driveTrain.followTrajectory(traj);
    ...
    public void runOuterOnlyLaunchPark(int waitInBetween){
        ...
        if (targetZone != HzGameFieldUltimateGoal.TARGET_ZONE.A) {
            ...
            traj = driveTrain.trajectoryBuilder(driveTrain.getPoseEstimate())
                    .lineToSplineHeading(new Pose2d(13, af * 35, Math.toRadians(af * 0))) // Near Target A
                    .build();
            driveTrain.followTrajectory(traj);
        }
        //*/
