...
public class JonVisual extends Visual {
    ...
    public double getTeamElement() {
        ...
        double[] hsv = new double[3];
        int lastBlue = -1; // keep track of the last yellow pixel seen
        int blackBounds[] = {-1, -1};
        ...
        for (int x = 0; x < currentFrame.getWidth(); x++) {
            ...
            if (215 < hsv[0] && hsv[0] < 260 && hsv[1] > 0.7) {
                if (lastBlue == -1 || x - lastBlue < 20) {
                    lastBlue = x;
                    currentFrame.setPixel(x, 0, 0x00FF00);
                }/* else if (blackBounds[1] == -1){
                    blackBounds[0] = lastBlue - 1;
                    blackBounds[1] = x - 1;
                    lastBlue = x;
                }*/
            } else if (lastBlue == -1) {
                currentFrame.setPixel(x, 0, 0xFF00FF);
            } else if (x - lastBlue < 20) {
        ...
        telemetry.addData("Blue Center", (blackBounds[1] + blackBounds[0]) / 2);
