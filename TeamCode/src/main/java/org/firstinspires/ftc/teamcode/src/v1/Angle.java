...
public class Angle {
    ...
    enum AngleType {
        ZERO_TO_360_CARTESIAN, ZERO_TO_360_HEADING, NEG_180_TO_180_HEADING, NEG_180_TO_180_CARTESIAN
    }
    ...
    //no other assumptions related to inputAngle value (can be -infinity ot infinity)
    ...
    public double convertAngleDouble (AngleType outputType) {
        ...
        if (type == outputType) {
            return wrapAngle(this.getAngle(), outputType); //was newAngle(angle, type)
        }
        ...
        if (sameNumericalSystem(type, outputType)) {
            ...
            double angleNewNumericalSystem = convertNumericalSystem(angle, type, numericalAndCoordinate(outputType, type)); //was type, output tupe
    ...
    //min return value is 0 and max is 180
