...
public class BlueRightAutoCycleTuned extends LinearOpMode {
    ...
    public static double dropX = -30, dropY = 0, dropA = toRadians(140), dropET = toRadians(310);
...
//                .addTemporalMarker(robot::done)
                .build();
        TrajectorySequence pickupTrajectory = robot.roadrun.trajectorySequenceBuilder(new Pose2d(-27, 5, toRadians(120)))
...
//                .splineTo(new Vector2d(-62.7,12), toRadians(180))
//
                .setReversed(false)
                .splineToSplineHeading(new Pose2d(-62.9, 13, toRadians(180)), toRadians(180))
                .addTemporalMarker(()->{robot.done(); robot.roadrun.breakFollowing();})
                .build();
        ArrayList<TrajectorySequence> dropTrajectory = new ArrayList<>();
        for(int i=0;i<5;i++){
            dropTrajectory.add(robot.roadrun.trajectorySequenceBuilder(new Pose2d(-62.9,13,Math.toRadians(180)))
                    .setReversed(true)
