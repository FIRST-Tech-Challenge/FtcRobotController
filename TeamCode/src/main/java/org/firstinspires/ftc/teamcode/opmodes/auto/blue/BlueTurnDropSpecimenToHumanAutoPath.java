...
@Autonomous(name = "BlueTurnDropSpecimenToHumanAutoPath", group = "Blue Alliance", preselectTeleOp = "RobotCentricDrive")
@Disabled
public class BlueTurnDropSpecimenToHumanAutoPath extends LinearOpMode {
    @Override
        ...
        if (opModeIsActive()) {
            ...
            Actions.runBlocking(
                    drive.actionBuilder(new Pose2d(-24, 60, Math.toRadians(-90)))
                            .waitSeconds(1)
                            .splineToSplineHeading(new Pose2d(-36, 40, Math.toRadians(-135)), -45)
                            .waitSeconds(1)
