...
public class Autonomous {
    ...
    public StateMachine AutoFull = getStateMachine(autoStage)
            ...
            .addSingleState(() -> robot.luncher.hookOn()) // makes sure the hook is down properly
            .addState(() -> robot.isBlue ?
                    robot.turret.rotateIMUTurret(90,3) :
                    robot.turret.rotateIMUTurret(270,3))
            .addState(() -> (robot.luncher.setElbowTargetPos(600, 1)))
            .addTimedState(3f, () -> sample(),  () -> telemetry.addData("DELAY", "DONE"))
            .addState(() -> robot.luncher.toggleGripper())
            .addSingleState(() -> robot.luncher.hookOff()) // makes sure the hook is down properly
            .addState(() -> robot.luncher.setGripperSwivelRotation(robot.luncher.swivel_Front-100))
            .addState(() -> (robot.luncher.setElbowTargetPos(500, 1)))
            ...
            .addMineralState(skystoneStateProvider,
                    () -> robot.luncher.setGripperSwivelRotation(1450),
                    () -> true,
                    () -> robot.luncher.setGripperSwivelRotation(1700))

            .addMineralState(skystoneStateProvider,
                    () -> robot.luncher.extendToPosition(2200, 1),
                    () -> robot.luncher.extendToPosition(2210, 1),
                    () -> robot.luncher.extendToPosition(2200, 1))

            // drop and snap gripper
            .addState(() -> robot.luncher.setElbowTargetPos(30, 1))
...
//
            .addSimultaneousStates( //retract arm while driving forward - trying to get about the same speed so stone doesn't really move
                    () -> robot.luncher.setElbowTargetPos(30, 1),
                    ()-> robot.luncher.extendToPosition(robot.luncher.extendMin, 1), // maybe look at using retract articulation
                    //pull away from wall half a meter
                    () -> robot.isBlue ?
                    robot.driveIMUDistanceWithReset(.6,90,true,.470) :
            ...
            // retrieve stone
            .addSingleState(() -> robot.articulate(PoseUG.Articulation.retrieveStone))
            .addTimedState(1f, () -> telemetry.addData("DELAY", "STARTED"), () -> telemetry.addData("DELAY", "DONE"))
            ...
            .addState(() -> (robot.luncher.setElbowTargetPos(300, 1)))

            // deposit stone
            .addState(() -> robot.isBlue ?
                    robot.turret.rotateIMUTurret(90,3) :
                    robot.turret.rotateIMUTurret(270,3))
            .addState(() -> robot.luncher.extendToPosition(1050, 1))
            .addTimedState(1f, () -> telemetry.addData("DELAY", "STARTED"), () -> telemetry.addData("DELAY", "DONE"))
            .addSingleState(() -> robot.articulate(PoseUG.Articulation.retractFromTower))
            ...
            .addTimedState(1f, () -> telemetry.addData("DELAY", "STARTED"), () -> telemetry.addData("DELAY", "DONE"))
            .addSingleState(() -> robot.luncher.hookOn())
            ...
            .addSingleState(() -> robot.luncher.setExtendABobLengthMeters(.9))
            .build();

    public StateMachine walkOfShame = getStateMachine(autoStage)
            .addState(() -> robot.luncher.setElbowTargetPos(robot.luncher.elbowMin-10, 1))
            .addSingleState(() -> robot.luncher.setExtendABobLengthMeters(0.25))
            .build();

    public StateMachine autoMethodTesterTool = getStateMachine(autoStage) // I do actually use this, do not delete
            .addSingleState(() -> robot.articulate(PoseUG.Articulation.autoRotateToFaceStone))
            .build();
