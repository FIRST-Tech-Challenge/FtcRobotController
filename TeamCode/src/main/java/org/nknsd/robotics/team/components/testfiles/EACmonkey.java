...
public class EACmonkey implements NKNComponent {
    ...
    private TestsOne currentTestOne = TestsOne.DO_NOTHING;
    private TestsTwo currentTestTwo = TestsTwo.DO_NOTHING;
    private int testNumber;

    private enum TestsOne {
        DO_NOTHING(1000),
        SERVO_FORWARD(3000),
        ROTATION_0_5(3000),
        ROTATION_1(3000),
        ...
        TestsOne(long durationMilli) {
            this.durationMilli = durationMilli;
        }
    }

    private enum TestsTwo {
        DO_NOTHING(1000),
        ROTATION_0(3000),
        SERVO_FORWARD(3000),
        ROTATION_0_5(3000),
    ...
    public void loop(ElapsedTime runtime, Telemetry telemetry) {
        ...
        if (testNumber == 1) {
            if (runtime.time(TimeUnit.MILLISECONDS) - testStartTime > currentTestOne.durationMilli) {
                // Get which test we're on
                int testIndex = currentTestOne.ordinal() + 1;
                // If outside the array's length, we need to loop back to the beginning
                if (testIndex >= TestsOne.values().length) {
                    //Reset to beginning of tests
                    testIndex = 0;
                }
                currentTestOne = TestsOne.values()[testIndex];
                testStartTime = runtime.time(TimeUnit.MILLISECONDS);
                testNumber = 2;
            }
            runTestOne(currentTestOne);

        } else if (testNumber == 2) {
            if (runtime.time(TimeUnit.MILLISECONDS) - testStartTime > currentTestTwo.durationMilli) {
                // Get which test we're on
                int testIndex = currentTestTwo.ordinal() + 1;
                // If outside the array's length, we need to loop back to the beginning
                if (testIndex >= TestsTwo.values().length) {
                    //Reset to beginning of tests
                    testIndex = 0;
                }
                currentTestTwo = TestsTwo.values()[testIndex];
                testStartTime = runtime.time(TimeUnit.MILLISECONDS);
                testNumber = 1;
            }
            runTestTwo(currentTestTwo);
        }
    }
    ...
    public void doTelemetry(Telemetry telemetry) {
        if (testNumber == 1) {
            telemetry.addData("Test One", currentTestOne.name());
        } else {
            telemetry.addData("Test Two", currentTestTwo.name());
        }
    }

    private void runTestOne(TestsOne tests) {
        switch (tests) {
            case DO_NOTHING:
    ...
    private void runTestTwo(TestsTwo tests) {
        switch (tests) {
            case DO_NOTHING:
                intakeServoHandler.setServoPower(0);
                break;
