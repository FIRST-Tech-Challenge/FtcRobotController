...
//            telemetry.addData("linear slide encoder",  + linearSlideMotor.getCurrentPosition());
            ...
            switch (outtakeState){
                case ReadyToTransfer:
                    if (gamepad1.x && hookPosition==HookPosition.CLOSED){ // if press x and hook is closed, open hook
                    ...
                    if (gamepad1.x && hookPosition == HookPosition.OPEN){ // closes hook with x
                        if(outtakeElapsedTime == null || outtakeElapsedTime.time(TimeUnit.MILLISECONDS)>300){
                ...
                case MoveToTransfer:
                    ...
                    break;
                case ReadyToDrop:
                    if (gamepad1.x && hookPosition==HookPosition.CLOSED){
                        if (outtakeElapsedTime==null || outtakeElapsedTime.time(TimeUnit.MILLISECONDS)>300) {
                            outtakeElapsedTime = new ElapsedTime();
                            hookPosition = HookPosition.OPEN;
                            outtakeHook.setPosition(CSCons.openHook);
                    ...
                    if (gamepad1.x && hookPosition == HookPosition.OPEN){
                        if(outtakeElapsedTime == null || outtakeElapsedTime.time(TimeUnit.MILLISECONDS)>300){
                            outtakeElapsedTime = closeHook();
                        } else {
                            outtakeHook.setPosition(CSCons.openHook);
