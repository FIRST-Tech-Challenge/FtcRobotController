...
public class PostSeason2020to21 {
    ...
    public PostSeason2020to21(CompBot bot) {
        this.bot = bot;
    ...
    public boolean moveTowardsTarget(VectorF target, Orientation angle) {
        ...
        if (botVis.targetVisible) {
            ...
            onTarget = ((Math.abs(angleDelta) < angleDeadband)  && (Math.abs(robotDelta.get(1)) < targetDeadband)  && (Math.abs(robotDelta.get(0)) < targetDeadband));
        }
